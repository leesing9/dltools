...
def getTasksInfo():
    ...
    if r.ok:
        ...
        for jobId, _ in taskInfo['jobSize'].items():
            _, labelType, labelClass = taskInfo['taskName'][taskInfo['jobTaskId'][jobId]].split('_')[:3]
...
def getAssignCR():
    ...
    for taskId, count in taskInfo['taskFrameCount'].items():
        _, labelType, labelClass = taskInfo['taskName'][taskId].split('_')[:3]
        if labelType=='Bbox':
...
async def mainSchedule():
    ...
    sched.add_job(logWorkload, 'interval', hours=1,timezone=timezone('Asia/Seoul'), id=f"logging", start_date=startTime)
    sched.add_job(runAssign,'interval', minutes=10, timezone=timezone('Asia/Seoul'), id=f"assign")
...
def get_sesstion(base_url, username, password):
